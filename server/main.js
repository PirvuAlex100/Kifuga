...
socket.on('scribbleGuess', (guess) => {
  ...
  if (guess.trim().toLowerCase().includes(game.word.toLowerCase())) {
    ...
    io.to(result.code).emit('scribbleCorrect', { guesser: socket.id, word: game.word });
    io.to(result.code).emit('roomUpdate', getRoomState(result.code));
...
socket.on('storyChooseGenre', (genre) => {
  ...
  io.to(result.code).emit('storyGenreChosen', { genre });
  game.turnOrder.forEach(id => {
    io.to(id).emit('roleAssignment', {
  role: id === game.turnOrder[game.currentTurnIndex] ? 'writer' : 'waiting',
  genre: game.genre,
  timer: game.timer,
...
socket.on('storyPassGenre', () => {
  ...
  if (game.turnOrder[nextIndex] === game.genrePicker) {
    ...
    game.genrePicker = null;
    io.to(result.code).emit('storyGenreChosen', { genre: game.genre });
    game.turnOrder.forEach(id => {
      io.to(id).emit('roleAssignment', {
        role: id === game.turnOrder[game.currentTurnIndex] ? 'writer' : 'waiting',
        genre: game.genre,
...
const roomName = (typeof data === 'string' ? `${playerName}'s Room` : (data.roomName?.trim() || `${playerName}'s Room`));
...
if (room.kickedNames && room.kickedNames.has(playerName)) {
  ...
  if (gameId === 'search-engine') {
    ...
    if (!initialGenre) {
      ...
      const playerColors = assignPlayerColors(shuffled);
      shuffled.forEach((id, i) => {
        io.to(id).emit('roleAssignment', {
          role: i === 0 ? 'writer' : 'waiting',
          genre: initialGenre,
          timer,
          turnOrder: shuffled,
          colors: playerColors,
...
function endScribbleRound(code, room, correct) {
  ...
  if (game.round < game.maxRounds) {
    ...
    io.to(code).emit('scribbleGameOver', { scores: game.scores });
    setTimeout(() => {
      room.gameActive = false;
      room.currentGame = null;
      room.gameData = null;
      for (const p of Object.values(room.players)) p.score = 0;
      io.to(code).emit('gameEnded', { name: 'Scribble' });
