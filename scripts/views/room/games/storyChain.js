...
function renderStoryChain(socket, gameState, currentPlayers) {
  ...
  socket.on('storyGenreChosen', ({ genre, turnOrder: newTurnOrder, colors, currentTurn }) => {
  storyGenre = genre;
    ...
    gameState.innerHTML = `
      <div class="flex flex-col h-full p-6 items-center justify-center text-center relative z-10">
        ...
        <div class="bg-amber-50/90 backdrop-blur-sm rounded-2xl p-6 shadow-inner max-w-lg w-full max-h-60 overflow-y-auto text-sm text-left space-y-2 mb-6 border border-amber-300/60"
             style="font-family: 'Georgia', serif; color: #3b2b1a; background: rgba(100,108,255, 0.05);">
...
function renderStoryWriterUI(socket, gameState, currentPlayers) {
  ...
  gameState.innerHTML = `
    ...
    <div class="relative flex flex-col h-full p-4 z-10">
      ...
      <div id="storyContent" class="flex-1 backdrop-blur-sm rounded-2xl p-5 shadow-inner overflow-y-auto mb-4 text-sm leading-relaxed border border-amber-300/60"
           style="font-family: 'Georgia', serif; color: #3b2b1a; background: rgba(100,108,255, 0.05);">
...
function renderStoryLines(lines) {
  ...
  return lines.map((l, i) =>
    `<span style="color: ${playerColors[l.playerId] || '#E05D5D'}; text-shadow: 1px 1px 3px gray; font-size: 1.5rem;">${escapeHtml(l.text)}</span>${i < lines.length - 1 ? ' ' : ''}`
  ).join('');
}
